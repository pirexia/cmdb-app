...
class User
{
    ...
    {
        try {
            $stmt = $this->db->prepare("SELECT id, nombre_usuario, password_hash, email, id_rol, activo, id_fuente_usuario, fuente_login_nombre FROM usuarios WHERE nombre_usuario = :username"); // Añadir nuevas columnas
    ...
    {
        try {
            $stmt = $this->db->prepare("SELECT id, nombre_usuario, password_hash, email, id_rol, activo, id_fuente_usuario, fuente_login_nombre FROM usuarios WHERE email = :email"); // Añadir nuevas columnas
     ...
     * @param string $username
     * @param string|null $passwordHash (puede ser null para usuarios no locales)
     * @param string|null $email (puede ser null para usuarios no locales) <-- ¡CAMBIO AQUÍ!
     * @param int $roleId
     * @param bool $activo
     ...
     */
    public function createUser(string $username, ?string $passwordHash, ?string $email, int $roleId, bool $activo, int $sourceId, string $sourceName) // <-- ¡AÑADE '?' ANTES DE string PARA $email!
    {
        try {
            $stmt = $this->db->prepare("INSERT INTO usuarios (nombre_usuario, password_hash, email, id_rol, activo, id_fuente_usuario, fuente_login_nombre) VALUES (:username, :password_hash, :email, :id_rol, :activo, :id_fuente_usuario, :fuente_login_nombre)");
            $stmt->bindParam(':username', $username, PDO::PARAM_STR);
            $stmt->bindValue(':password_hash', $passwordHash, ($passwordHash === null ? PDO::PARAM_NULL : PDO::PARAM_STR)); // Ya corregido
            $stmt->bindValue(':email', $email, ($email === null ? PDO::PARAM_NULL : PDO::PARAM_STR)); // <-- ¡CAMBIO AQUÍ PARA BINDING NULL!
     ...
     */
    public function updateUser(int $id, string $username, string $email, int $roleId, bool $activo, int $sourceId, string $sourceName) // <-- ¡NUEVOS ARGUMENTOS!
    {
        try {
            $stmt = $this->db->prepare("UPDATE usuarios SET nombre_usuario = :username, email = :email, id_rol = :id_rol, activo = :activo, id_fuente_usuario = :id_fuente_usuario, fuente_login_nombre = :fuente_login_nombre WHERE id = :id"); // <-- ¡NUEVAS COLUMNAS!
            $stmt->bindParam(':username', $username, PDO::PARAM_STR);
            $stmt->bindParam(':email', $email, PDO::PARAM_STR);
            $stmt->bindParam(':id_rol', $roleId, PDO::PARAM_INT);
            $stmt->bindValue(':activo', $activo ? 1 : 0, PDO::PARAM_INT);
            $stmt->bindParam(':id_fuente_usuario', $sourceId, PDO::PARAM_INT); // <-- NUEVO
            $stmt->bindParam(':fuente_login_nombre', $sourceName, PDO::PARAM_STR); // <-- NUEVO
            $stmt->bindParam(':id', $id, PDO::PARAM_INT);
            $stmt->execute();
